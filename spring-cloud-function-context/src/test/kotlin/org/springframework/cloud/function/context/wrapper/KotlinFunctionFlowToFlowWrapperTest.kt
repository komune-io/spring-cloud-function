...
class KotlinFunctionFlowToFlowWrapperTest {
    ...
    // Sample function that transforms a Flow to another Flow
    private val sampleFunction: (Flow<String>) -> Flow<String> = { flow ->
        flow.map { it.uppercase() }
    }

    @Test
    fun `test isValid with valid flow to flow function type`() {
        // Given
        ...
        // When
        val result = KotlinFunctionFlowToFlowWrapper.isValid(functionType, types)
    ...
    @Test
    fun `test isValid with invalid function type (not flow to flow)`() {
        // Given
        val functionType = typeOf<(String) -> Flow<String>>().javaType
        val types = arrayOf<Type>(
    ...
    fun `test asRegistrationFunction creates wrapper correctly`() {
        ...
        // When
        val wrapper = KotlinFunctionFlowToFlowWrapper.asRegistrationFunction(functionName, sampleFunction, types)
    ...
    fun `test constructor with type parameter`() {
        ...
        // When
        val wrapper = KotlinFunctionFlowToFlowWrapper(sampleFunction, type, functionName)
